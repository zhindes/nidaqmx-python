...
class ADCTimingMode(Enum):
    ...
    CUSTOM = 10137  #: Use DAQmx_AI_ADCCustomTimingMode to specify a custom value controlling the tradeoff between speed and resolution.
...
class BridgeConfiguration(Enum):
    FULL_BRIDGE = 10182  #: Sensor is a full bridge. If you set DAQmx_AI_Excit_UseForScaling to TRUE, NI-DAQmx divides the measurement by the excitation value. Many sensors scale data to native units using scaling of volts per excitation.
    HALF_BRIDGE = 10187  #: Sensor is a half bridge. If you set DAQmx_AI_Excit_UseForScaling to TRUE, NI-DAQmx divides the measurement by the excitation value. Many sensors scale data to native units using scaling of volts per excitation.
    QUARTER_BRIDGE = 10270  #: Sensor is a quarter bridge. If you set DAQmx_AI_Excit_UseForScaling to TRUE, NI-DAQmx divides the measurement by the excitation value. Many sensors scale data to native units using scaling of volts per excitation.
...
class CJCSource(Enum):
    ...
    CONST_VAL = 10116  #: You must specify the cold-junction temperature.
    CHAN = 10113  #: Use a channel for cold-junction compensation.
...
class CountDirection(Enum):
    ...
    EXT_CONTROLLED = 10326  #: The state of a digital line controls the count direction. Each counter has a default count direction terminal.
...
class CurrentShuntResistorLocation(Enum):
    ...
    EXTERNAL = 10167  #: Use a shunt resistor external to the device. You must specify the value of the shunt resistor by using DAQmx_AI_CurrentShunt_Resistance.
    DEFAULT = -1
...
class DataJustification(Enum):
    RIGHT_JUSTIFIED = 10279  #: Samples occupy the lower bits of the integer.
    LEFT_JUSTIFIED = 10209  #: Samples occupy the higher bits of the integer.
...
class DataTransferMechanism(Enum):
    ...
    PROGRAMMED_IO = 10264  #: Data transfers take place when you call an NI-DAQmx Read function or an NI-DAQmx Write function.
...
class DeassertCondition(Enum):
    ...
    ONBRD_MEM_CUSTOM_THRESHOLD = 12577  #: Deassert the signal when the amount of space available in the onboard memory is below the value specified with DAQmx_Exported_RdyForXferEvent_DeassertCondCustomThreshold.
...
class DigitalWidthUnits(Enum):
    SAMP_CLK_PERIODS = 10286  #: Complete periods of the Sample Clock.
...
class EncoderZIndexPhase(Enum):
    A_HIGH_B_HIGH = 10040  #: Reset the measurement when signal A and signal B are high.
    A_HIGH_B_LOW = 10041  #: Reset the measurement when signal A is high and signal B is low.
    A_LOW_B_HIGH = 10042  #: Reset the measurement when signal A is low and signal B high.
    A_LOW_B_LOW = 10043  #: Reset the measurement when signal A and signal B are low.
...
class ExportActions(Enum):
    ...
    LVL = 10210  #: The exported Sample Clock goes high at the beginning of the sample and goes low when the last AI Convert begins.
    INTERLOCKED = 12549  #: Handshake Event deasserts after the Handshake Trigger asserts, plus the amount of time specified with DAQmx_Exported_HshkEvent_Interlocked_DeassertDelay.
...
class GpsSignalType(Enum):
    ...
    PPS = 10080  #: Use the PPS synchronization method. The GPS receiver sends one synchronization pulse per second, but does not send any timing information. The timestamp measurement returns the number of seconds that elapsed since the device powered up unless you set DAQmx_CI_Timestamp_InitialSeconds.
    NONE = 10230  #: Do not synchronize the counter to a GPS receiver. The timestamp measurement returns the number of seconds that elapsed since the device powered up unless you set DAQmx_CI_Timestamp_InitialSeconds.


class GroupBy(Enum):
    CHANNEL = 0  #: Group by Channel
    SCAN_NUMBER = 1  #: Group by Scan Number
...
class InputDataTransferCondition(Enum):
    ...
    ONBRD_MEM_CUSTOM_THRESHOLD = 12577  #: Transfer data from the device when the number of samples specified with DAQmx_AI_DataXferCustomThreshold are in the device FIFO.
...
class OverwriteMode(Enum):
    OVERWRITE_UNREAD_SAMPS = 10252  #: When an acquisition encounters unread data in the buffer, the acquisition continues and overwrites the unread samples with new ones. You can read the new samples by setting DAQmx_Read_RelativeTo to DAQmx_Val_MostRecentSamp and setting DAQmx_Read_Offset to the appropriate number of samples.
    DO_NOT_OVERWRITE_UNREAD_SAMPS = 10159  #: The acquisition stops when it encounters a sample in the buffer that you have not read.
...
class RTDType(Enum):
    ...
    CUSTOM = 10137  #: You must use DAQmx_AI_RTD_A, DAQmx_AI_RTD_B, and DAQmx_AI_RTD_C to supply the coefficients for the Callendar-Van Dusen equation.
...
class RawDataCompressionType(Enum):
    ...
    LOSSY_LSB_REMOVAL = 12556  #: Remove unused bits from samples. Then, if necessary, remove bits from samples until the samples are the size specified with DAQmx_AI_LossyLSBRemoval_CompressedSampSize. This compression type limits resolution to the specified sample size.
...
class ReadRelativeTo(Enum):
    ...
    MOST_RECENT_SAMP = 10428  #: Start reading samples relative to the next sample acquired. For example, use this value and set DAQmx_Read_Offset to -1 to read the last sample acquired.


class RegenerationMode(Enum):
    ALLOW_REGEN = 10097  #: Allow NI-DAQmx to regenerate samples that the device previously generated. When you choose this value, the write marker returns to the beginning of the buffer after the device generates all samples currently in the buffer.
    DO_NOT_ALLOW_REGEN = 10158  #: Do not allow NI-DAQmx to regenerate samples the device previously generated. When you choose this value, NI-DAQmx waits for you to write more samples to the buffer or until the timeout expires.
...
class SampleClockActiveOrInactiveEdgeSelection(Enum):
    SAMP_CLK_ACTIVE_EDGE = 14617  #: Active edges.
    SAMP_CLK_INACTIVE_EDGE = 14618  #: Inactive edges.
...
class Slope(Enum):
    RISING_SLOPE = 10280  #: Trigger on the rising slope of the signal.
    FALLING_SLOPE = 10171  #: Trigger on the falling slope of the signal.
...
class ThermocoupleType(Enum):
    J_TYPE_TC = 10072  #: J-type thermocouple.
    K_TYPE_TC = 10073  #: K-type thermocouple.
    N_TYPE_TC = 10077  #: N-type thermocouple.
    R_TYPE_TC = 10082  #: R-type thermocouple.
    S_TYPE_TC = 10085  #: S-type thermocouple.
    T_TYPE_TC = 10086  #: T-type thermocouple.
    B_TYPE_TC = 10047  #: B-type thermocouple.
    E_TYPE_TC = 10055  #: E-type thermocouple.
...
class Timescale(Enum):
    HOST_TIME = 16126  #: Use the host device.
    IO_DEVICE_TIME = 16127  #: Use the I/O device.
...
class TriggerType(Enum):
    ANLG_EDGE = 10099  #: Trigger when an analog signal signal crosses a threshold.
    ANLG_MULTI_EDGE = 16108  #: Trigger when any of the configured analog signals cross their respective thresholds.
    DIG_EDGE = 10150  #: Trigger on the rising or falling edge of a digital signal.
    DIG_PATTERN = 10398  #: Trigger when digital physical channels match a digital pattern.
    ANLG_WIN = 10103  #: Trigger when an analog signal enters or leaves a range of values. The range is in the units of the measurement.
    ...
    ANLG_LVL = 10101  #: Pause the measurement or generation while an analog signal is above or below a level.
    DIG_LVL = 10152  #: Pause the measurement or generation while a digital signal is at either a high or low state.
...
class WaitMode(Enum):
    ...
    SLEEP = 12547  #: Check for available buffer space once per the amount of time specified in DAQmx_Write_SleepTime.
    WAIT_FOR_INTERRUPT = 12523  #: Check for Sample Clock pulses when the system receives an interrupt service request. This mode is the most CPU efficient, but results in lower possible sampling rates.
...
class WatchdogAOOutputType(Enum):
    VOLTAGE = 10322  #: Voltage output.
    CURRENT = 10134  #: Current output.
    NO_CHANGE = 10160  #: Expiration does not affect the port. Do not change the state of any lines in the port, and do not lock the port.
...
class WatchdogControlAction(Enum):
    RESET_TIMER = 0  #: Reset Timer
    CLEAR_EXPIRATION = 1  #: Clear Expiration


class WindowTriggerCondition(Enum):
